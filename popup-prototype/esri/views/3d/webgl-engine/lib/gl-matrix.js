...
(function (root, factory) {
  factory(root, true);
  factory(root, false);
}(glMatrix, function (root, singlePrecision) {
  "use strict";
...
*/
  ...
  if (typeof(Float32Array) != 'undefined'){
    MatrixArray = singlePrecision ? Float32Array : Array;
  }
...
*/
  ...
  vec3.max = function (vec1, vec2, dest) {     
    dest[0] = Math.max(vec1[0], vec2[0]);
    dest[1] = Math.max(vec1[1], vec2[1]);
    dest[2] = Math.max(vec1[2], vec2[2]);
    return dest;
  };

  vec3.min = function (vec1, vec2, dest) {     
    dest[0] = Math.min(vec1[0], vec2[0]);
    dest[1] = Math.min(vec1[1], vec2[1]);
    dest[2] = Math.min(vec1[2], vec2[2]);
    return dest;
  };
...
*/
  ...
  var tmpDir = vec3.create();
  var tmpDir2 = vec3.create();
  
  vec3.project = function(vec, vecA, vecB, dest) {
    if (!dest) dest = vec;
    
    vec3.direction(vecA, vecB, tmpDir);
    vec3.subtract(vec, vecA, tmpDir2);
    
    var dot = vec3.dot(tmpDir, tmpDir2);
  
    vec3.scale(tmpDir, dot, dest);
    vec3.add(dest, vecA, dest);
  };
...
*/
  ...
  mat4.createFromMatrixRowMajor = function (mat) {
    var dest = new MatrixArray(16);
    dest[0] = mat[0];
    dest[4] = mat[1];
    dest[8] = mat[2];
  ...
  mat4.createFromMatrix = function (mat) {
    var dest = new MatrixArray(16);
    dest[0] = mat[0];
    dest[1] = mat[1];
    dest[2] = mat[2];
...
*/
  ...
  mat4.setRowMajor = function (mat, dest) {
    dest[0] = mat[0];
    dest[4] = mat[1];
    dest[8] = mat[2];
    dest[12] = mat[3];
...
*/
  ...
  /**
   * Computes the scaling factor of the 3 axes and returns the largest one
   * @param {mat4} mat mat4 to rotate
   * @returns {number} The largest scaling factor described by mat
   */
  mat4.maxScale = function(mat) {
    var sx = Math.sqrt(mat[0] * mat[0] + mat[4] * mat[4] + mat[8]  * mat[8]);
    var sy = Math.sqrt(mat[1] * mat[1] + mat[5] * mat[5] + mat[9]  * mat[9]);
    var sz = Math.sqrt(mat[2] * mat[2] + mat[6] * mat[6] + mat[10] * mat[10]);
...
*/
  ...
  vec4.set4 = function (x, y, z, w, dest) {
    dest[0] = x;
    dest[1] = y;
    dest[2] = z;
    dest[3] = w;
...
*/
  ...
  var suffix = singlePrecision ? '' : 'd';
  
  root['glMath' + suffix] = glMath;
  root['vec2' + suffix] = vec2;
  root['vec3' + suffix] = vec3;
  root['vec4' + suffix] = vec4;
  root['mat2' + suffix] = mat2;
  root['mat3' + suffix] = mat3;
  root['mat4' + suffix] = mat4;
  root['quat4' + suffix] = quat4;
}));
