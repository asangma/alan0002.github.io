...
define([
  ...
  var SSAOHelperObscurance = function SSAOHelper(programRep, textureRep, gl) {
    ...
    var getNumSpiralTurns = function(numSamples) {
      if (numSamples < minDiscrepancyArray.length) {
    ...
    var projInfo = [0,0,0,0];

    this.computeSSAO= function(camera, fbo, sceneNearFar, depthFBO, normalFBO) {
      Util.assert(this.getEnableState());

      var width = depthFBO.getWidth();
      var height = depthFBO.getHeight();
      ...
      var viewportScaleX = 1.0;  // todo: scale for retina support
      var viewportScaleY = 1.0;
      var dimIScaled = width*viewportScaleX;
      var dimJScaled = height*viewportScaleY;


      ssaoFBO.bind();

      vec4.set(camera.viewport, viewportToRestore);

      // render ao term into ssao
      gl.viewport(0, 0, width, height);

      var ssAmbientObscurance = programRep.get(samples <= 8 ? "ssao8" : (samples <= 16 ? "ssao16" : (samples <= 32 ? "ssao32" : "ssao64")));

      var blurEdgeAware = programRep.get("blur");

      //ssAmbientObscurance.clearLookupTables(); //for firefox shader debugger

      ssAmbientObscurance.use();


      var projMatrix = camera.projectionMatrix;
      projInfo[0] =  -2.0 / (dimIScaled*projMatrix[0]); //m00
      projInfo[1] =  -2.0 / (dimJScaled*projMatrix[5]); //m11
      projInfo[2] =  ( 1.0 - projMatrix[2]) / projMatrix[0]; //m02 m00
      projInfo[3] =  ( 1.0 + projMatrix[6]) / projMatrix[5]; //m12 m11

      var isOrtho = (projMatrix[11] === 0);

      ssAmbientObscurance.uniform2f("rnmScale", width / noiseDim, height / noiseDim);

      //if (ctxVars.ssaoSamples == -1) {
      //  evaluateInteractiveSSAOSamples(ctx);
      //}

      ssAmbientObscurance.uniform3fv("pSphere", samples <= 8 ? rndVectors8 : (samples <= 16 ? rndVectors16 : (samples <= 32 ? rndVectors32 : rndVectors64)));

      ssAmbientObscurance.uniform1f("numSpiralTurns", getNumSpiralTurns(samples));

      ssAmbientObscurance.uniform4fv("projInfo", projInfo);
      var  projScale = 1.0/camera.computePixelSizeAtDist(1.0);
      ssAmbientObscurance.uniform1f("projScale",      projScale*viewportScaleX);
      ssAmbientObscurance.uniform2f("screenDimensions", dimIScaled, dimJScaled);

      if (!isOrtho) {
        ssAmbientObscurance.uniform2f("nearFar",camera.near,camera.far);
        ssAmbientObscurance.uniform2f("zScale", 1.0, 0.0);
      }
      else {
        ssAmbientObscurance.uniform2f("nearFar", camera.near,camera.far);
        ssAmbientObscurance.uniform2f("zScale", 0.0, -1.0);
      }

      //calc visually pleasing parameters based on heuristics
      var radiusScaled = radius*2.0;

      var midDepth = vec3.dist(camera.eye, camera.center);
      radiusScaled = camera.computePixelSizeAtDist(midDepth) * 20.0;
      radiusScaled = Math.max(0.1, radiusScaled);

      ssAmbientObscurance.uniform1f("radius", radiusScaled);
      ssAmbientObscurance.uniform1f("intensity", 4.0*attenuation/Math.pow(radiusScaled, 6.0));

      ssAmbientObscurance.uniform1i("rnm", 0);
      ssAmbientObscurance.uniform1i("normalMap", 1);
      ssAmbientObscurance.uniform1i("depthMap", 2);
      ...
      quadVBO.setPointers(ssAmbientObscurance);
      quadVBO.getLayout().enableVertexAttribArrays(gl, ssAmbientObscurance);  // NOTE setup only once ssao and blur program have the same vertex shader
      ...
      blurEdgeAware.changeShaderDefines(1,["RADIUS "+filterRadius]);

      var sigma      = (filterRadius+1.0) / 2.0;
      var inv_sigma2 = 1.0 / (2.0*sigma*sigma);
      ...
      gl.viewport(0, 0, dimIScaled / BLUR_F, dimJScaled / BLUR_F);


      blur0FBO.bind();
      blurEdgeAware.use(gl);
      blurEdgeAware.uniform2f("screenDimensions", dimIScaled, dimJScaled);

      blurEdgeAware.uniform1i("tex",      0);
      blurEdgeAware.uniform1i("normalMap",      1);
      blurEdgeAware.uniform1i("depthMap",      2);
      blurEdgeAware.uniform2f("blurSize", 0.0, BLUR_F*viewportScaleX/ dimJScaled);
      blurEdgeAware.uniform1i("radius", filterRadius);
      blurEdgeAware.uniform1f("g_BlurFalloff",      inv_sigma2);
      blurEdgeAware.uniform2f("nearFar",camera.near,camera.far);

      //remove edge awareness when far away
      if (midDepth>50000.0) {
        projScale = Math.max(0.0, projScale - (midDepth - 50000.0));
      }
      blurEdgeAware.uniform1f("projScale",      projScale);

      if (!isOrtho) {
        blurEdgeAware.uniform2f("zScale", 1.0, 0.0);
      }
      else {
        blurEdgeAware.uniform2f("zScale", 0.0, -1.0);
      }
      ...
      blurEdgeAware.uniform2f("blurSize", BLUR_F*viewportScaleY / dimIScaled, 0.0);

      blur1FBO.bind();
      gl.bindTexture(gl.TEXTURE_2D, blur0FBO.getTexture());
      gl.drawArrays(gl.TRIANGLE_STRIP, 0, quadVBO.getNum());

      quadVBO.getLayout().disableVertexAttribArrays(gl, ssAmbientObscurance);
    ...
    this.setUniforms = function(program) {
      ...
      //shaders directly use the ssao result in forward rendering
      //todo: switch to deferred for ssao and shadows?
      ...
      gl.bindTexture(gl.TEXTURE_2D, state ? blur1FBO.getTexture() : emptyTexture);
      gl.activeTexture(gl.TEXTURE0);
    ...
    this.drawQuad = function(proj) {
      ...
      gl.bindTexture(gl.TEXTURE_2D, blur1FBO.getTexture());
  ...
  SSAOHelperObscurance.loadShaders = function(snippets, shaderRep, programRep, gl) {
    ...
    var ssao = snippets.createFsSSAOSrcObscurance;
    ...
    var fsBlur = new GLSLShader(gl.FRAGMENT_SHADER, snippets.fsBlurEdgeAware, gl);
    ...
    var programBlur = new GLSLProgram(new Array(vsUVQuad, fsBlur), gl, ["RADIUS 4"]);
    ...
    programRep.add("blur", programBlur);
    delete snippets.samples;
  };

  return SSAOHelperObscurance;
});
