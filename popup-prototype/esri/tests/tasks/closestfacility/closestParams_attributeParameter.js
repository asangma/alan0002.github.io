dojo.provide("esri.tests.tasks.closestfacility.closestParams_attributeParameter");

dojo.require("esri.tasks.closestfacility");

var closestTask, closestParams;

doh.registerGroup("tasks.closestfacility.closestParams_attributeParameter", [{
    name: "closestParams_attributeParameter",
    timeout: 3000,
    runTest: function(){
        closestParams = new esri.tasks.ClosestFacilityParameters();
		
        closestParams.attributeParameter = [
          {
            attributeName: "Time",
        ...
        function showResults(solveResult){
            ...
            if (solveResult == null || solveResult.length == 0) {
                ...
                try {
                    ...
                    console.log(closestParams.attributeParameter);
                    doh.assertEqual("Time",closestParams.attributeParameter[0].attributeName);
					doh.assertEqual("20",closestParams.attributeParameter[1].value);
                    d.callback(true);
                } 
                catch (e) {
                    d.errback(e);
