dojo.provide("esri.tests.tasks.closestfacility.closest_directionTest");

dojo.require("esri.tasks.closestfacility");

var closestTask, closestParams;

doh.registerGroup("tasks.closestfacility.closest_directionTest", [{
    name: "closest_directionTest",
    timeout: 3000,
    runTest: function(){
        closestParams = new esri.tasks.ClosestFacilityParameters();
        closestParams.returnDirections = true;
        ...
        function showResults(solveResult){
            ...
            if (solveResult == null || solveResult.length == 0) {
                ...
                try {
					if(solveResult.directions[0].features.length>=1){
						doh.assertTrue(true);
					}
