dojo.provide("esri.tests.tasks.closestfacility.closest_facilitiesTest");

dojo.require("esri.tasks.closestfacility");

var closestTask, closestParams;

doh.registerGroup("tasks.closestfacility.closest_facilitiesTest", [{
    name: "closest_facilitiesTest",
    timeout: 3000,
    runTest: function(){
        closestParams = new esri.tasks.ClosestFacilityParameters();
        closestParams.returnFacilities = true;
		closestParams.returnRoutes = false;     
        ...
        function showResults(solveResult){
            ...
            if (solveResult == null || solveResult.length == 0) {
                ...
                try {
						doh.assertEqual(6,solveResult.facilities.length);
