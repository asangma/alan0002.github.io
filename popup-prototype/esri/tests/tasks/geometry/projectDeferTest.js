dojo.provide("esri.tests.tasks.geometry.projectDeferTest");

var geometryService;

doh.registerGroup("tasks.geometry.projectDeferTest", [
/**
  {
    name: "pointGraphic",
    timeout: 3000,
    runTest: function(t)
    {
      ...
      function showResults(graphics)
      {
        ...
        {
		  //console.log(dojo.toJson(graphics[0].toJSON()));
      ...
      //geometryService.project(graphics,outSR,showResults);//pre-2.0
	  geometryService.project(esri.getGeometries(graphics),outSR,showResults); // features needs to be an array of geometries
 ...
 
  {
    ...
    runTest: function(t)
    {
	  ...
	  **/
	  var expectedResultJSON = {"paths":[[[-122.686826282779,45.4705349944436],
	  [-122.687253249644,45.4705258915539],[-122.68747598129,45.4705154138976],
	  [-122.687640093146,45.4705393034179],[-122.687697659215,45.4706213722485],
	  [-122.687720614608,45.4707188688393],[-122.687690937278,45.4708207467957],
	  [-122.687544639631,45.4709960712593]]],"spatialReference":{"wkid":4326}};
      ...
      //geometryService.project(graphics,outSR,showResults);//pre-2.0
	  var def = geometryService.project(esri.getGeometries(graphics),outSR); // features needs to be an array of geometries
...

  {
    ...
    runTest: function(t)
    {
	 ...
	 [-122.75956788494,45.4519925662037],[-122.75956891995,45.4519695664942],
	 [-122.759567843841,45.4519424926515],[-122.759723779017,45.4519425080531],
	 [-122.759724950558,45.4519719808423],[-122.759724467344,45.4520088606302],
	 [-122.759725694655,45.452039704221],[-122.759725150121,45.4520750413778],
	 [-122.759567286056,45.4520755779633],[-122.759567885174,45.4520416116347]]],
	 "spatialReference":{"wkid":4326}};
	 **/
     var expectedResultJSON = {"rings": [[[-122.759567885174, 45.4520416116347], 
		[-122.75956725175, 45.4520256753158], [-122.759639362884, 45.4520254562882], 
		[-122.759640663478, 45.4520091389122], [-122.759641957278, 45.4519926506325], 
		[-122.75956788494, 45.4519925662037], [-122.75956891995, 45.4519695664942], 
		[-122.759567843841, 45.4519424926515], [-122.759723779017, 45.4519425080531], 
		[-122.759724950558, 45.4519719808423], [-122.759724467344, 45.4520088606302], 
		[-122.759725694655, 45.452039704221], [-122.759725150121, 45.4520750413778], 
