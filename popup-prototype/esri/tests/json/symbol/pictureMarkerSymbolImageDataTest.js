dojo.provide("esri.tests.json.symbol.pictureMarkerSymbolImageDataTest");


var pictureMarkerSymbolImageDataJson, pictureMarkerSymbolImageData;
function hasUndefined(queue){

    var obj = queue.shift(), value;

    if (dojo.isArray(obj)) {
        for (var i = 0; i < obj.length; i++) {
            value = obj[i];
            console.log("value in array:"+value);
            if (value === undefined) {

                return true;
            }
            else
            if (dojo.isObject(value)) {
                queue.push(value);

            }
        }
    }
    else { // assumed object
        for (var prop in obj) {
            value = obj[prop];
            console.log("value in object:"+value);
            if (value === undefined) {
                console.log(prop+"_"+dojo.toJson(value));
                return true;
            }
            else
            if (dojo.isObject(value)) {
                queue.push(value);
            }
        }
    }

    return false;
}
doh.registerGroup("json.symbol.pictureMarkerSymbolImageDataTest", [{
    name: "testPictureMarkerSymbolImageData",
    timeout: 3000,
    runTest: function(t){
		t.assertEqual(dojox.gfx.pt2px(19.5),pictureMarkerSymbolImageData.height);
		t.assertEqual(dojox.gfx.pt2px(19.5),pictureMarkerSymbolImageData.width);
		t.assertEqual("471E7E31",pictureMarkerSymbolImageData.url);
		t.assertEqual(0,pictureMarkerSymbolImageData.angle);
		t.assertEqual(20,pictureMarkerSymbolImageData.size);
		
		t.assertEqual(0,pictureMarkerSymbolImageData.xoffset);
		t.assertEqual(0,pictureMarkerSymbolImageData.yoffset);
		t.assertEqual("esriPMS",pictureMarkerSymbolImageData.type);
		
		t.assertEqual(null,pictureMarkerSymbolImageData.color);
				
        
    }

}, {
    name: "testToJson",
    timeout: 3000,
    runTest: function(t){
		console.log(dojo.toJson(pictureMarkerSymbolImageData.toJSON()));
        t.assertEqual(pictureMarkerSymbolImageDataJson, pictureMarkerSymbolImageData.toJSON());
        //added undefined test
        var json = pictureMarkerSymbolImageData.toJSON();
        var q = [json], found = false;

        do {
            found = hasUndefined(q);
        }
        while (!found && q.length > 0);

        if (found) {
            console.error("Test Failed");
        }
        else {
            console.info("Test Passed");
        }
        console.log(found);
        t.assertFalse(found);
    }
}], function()//setUp()
{

    pictureMarkerSymbolImageDataJson = {
	"type" : "esriPMS", 
	"url" : "471E7E31", 
	"imageData" : "", 
	"contentType" : "image/png", 
	"color" : null, 
	"width" : 19.5, 
	"height" : 19.5, 
	"angle" : 0, 
	"xoffset" : 0, 
	"yoffset" : 0
};

    pictureMarkerSymbolImageData = new esri.symbol.PictureMarkerSymbol(pictureMarkerSymbolImageDataJson);
    
    
}, function()//tearDown
{
    pictureMarkerSymbolImageData = null;
});
