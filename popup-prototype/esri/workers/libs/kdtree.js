...
(function(){
  ...
  function kdTree(points, metric, dimensions) {
    ...
    this.nearest = function (point, maxNodes, filter, maxDistance) {
      ...
      //if(!filter) filter = function(v) { return true; } - mpriour: only use function when need to. each function adds to processing time
      ...
      function nearestSearch(node) {
        ...
        if (node.right === null && node.left === null) {
          if ((bestNodes.size() < maxNodes || ownDistance < bestNodes.peek()[1])) {
            if(!filter || filter(node.obj)){ //mpriour - only call filter when defined
