define(
[
  "dojo/_base/array",
  "dojo/_base/lang",
  "dojo/number",
  "dojo/_base/Color",

  "../../core/lang",
  "../../renderers/ClassBreaksRenderer",
  "../../renderers/UniqueValueRenderer",
  "../../symbols/SimpleMarkerSymbol",
  "../../symbols/SimpleLineSymbol",
  "../../symbols/SimpleFillSymbol"
],
function(
  array, lang, number, Color,
  esriLang, ClassBreaksRenderer, UniqueValueRenderer, SimpleMarkerSymbol, SimpleLineSymbol, SimpleFillSymbol
) {
    var normTypes = {
      "field": "esriNormalizeByField",
      "log": "esriNormalizeByLog",
      "percent-of-total": "esriNormalizeByPercentOfTotal"
    };
    var methods = {
      "natural-breaks": "esriClassifyNaturalBreaks",
      "equal-interval": "esriClassifyEqualInterval",
      "quantile": "esriClassifyQuantile",
      "standard-deviation": "esriClassifyStandardDeviation",
      "geometrical-interval": "esriClassifyGeometricalInterval"
    };
    var namedColorRamps = [{
      "imageData": "",
      "colors": [[239, 243, 255, 255], [189, 215, 231, 255], [107, 174, 214, 255], [49, 130, 189, 255], [8, 81, 156, 255]],
      "name": "Blues",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Single/Blues.png"
}, {
      "imageData": "",
      "colors": [[237, 248, 233, 255], [186, 228, 179, 255], [116, 196, 118, 255], [49, 163, 84, 255], [0, 109, 44, 255]],
      "name": "Greens",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Single/Greens.png"
}, {
      "imageData": "",
      "colors": [[247, 247, 247, 255], [204, 204, 204, 255], [150, 150, 150, 255], [99, 99, 99, 255], [37, 37, 37, 255]],
      "name": "Grays",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Single/Grays.png"
}, {
      "imageData": "",
      "colors": [[254, 237, 222, 255], [253, 190, 133, 255], [253, 141, 60, 255], [230, 85, 13, 255], [166, 54, 3, 255]],
      "name": "Oranges",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Single/Oranges.png"
}, {
      "imageData": "",
      "colors": [[242, 240, 247, 255], [203, 201, 226, 255], [158, 154, 200, 255], [117, 107, 177, 255], [84, 39, 143, 255]],
      "name": "Purples",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Single/Purples.png"
}, {
      "imageData": "",
      "colors": [[254, 229, 217, 255], [252, 174, 145, 255], [251, 106, 74, 255], [222, 45, 38, 255], [165, 15, 21, 255]],
      "name": "Reds",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Single/Reds.png"
}, {
      "imageData": "",
      "colors": [[237, 248, 251, 255], [178, 226, 226, 255], [102, 194, 164, 255], [44, 162, 95, 255], [0, 109, 44, 255]],
      "name": "BuGn",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/BuGn.png"
}, {
      "imageData": "",
      "colors": [[237, 248, 251, 255], [179, 205, 227, 255], [140, 150, 198, 255], [136, 86, 167, 255], [129, 15, 124, 255]],
      "name": "BuPu",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/BuPu.png"
}, {
      "imageData": "",
      "colors": [[240, 249, 232, 255], [186, 228, 188, 255], [123, 204, 196, 255], [67, 162, 202, 255], [8, 104, 172, 255]],
      "name": "GnBu",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/GnBu.png"
}, {
      "imageData": "",
      "colors": [[254, 240, 217, 255], [253, 204, 138, 255], [252, 141, 89, 255], [227, 74, 51, 255], [179, 0, 0, 255]],
      "name": "OrRd",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/OrRd.png"
}, {
      "imageData": "",
      "colors": [[241, 238, 246, 255], [189, 201, 225, 255], [116, 169, 207, 255], [43, 140, 190, 255], [4, 90, 141, 255]],
      "name": "PuBu",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/PuBu.png"
}, {
      "imageData": "",
      "colors": [[246, 239, 247, 255], [189, 201, 225, 255], [103, 169, 207, 255], [28, 144, 153, 255], [1, 108, 89, 255]],
      "name": "PuBuGn",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/PuBuGn.png"
}, {
      "imageData": "",
      "colors": [[241, 238, 246, 255], [215, 181, 216, 255], [223, 101, 176, 255], [221, 28, 119, 255], [152, 0, 67, 255]],
      "name": "PuRd",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/PuRd.png"
}, {
      "imageData": "",
      "colors": [[254, 235, 226, 255], [251, 180, 185, 255], [247, 104, 161, 255], [197, 27, 138, 255], [122, 1, 119, 255]],
      "name": "RdPu",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/RdPu.png"
}, {
      "imageData": "",
      "colors": [[255, 255, 204, 255], [194, 230, 153, 255], [120, 198, 121, 255], [49, 163, 84, 255], [0, 104, 55, 255]],
      "name": "YlGn",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/YlGn.png"
}, {
      "imageData": "",
      "colors": [[255, 255, 204, 255], [161, 218, 180, 255], [65, 182, 196, 255], [44, 127, 184, 255], [37, 52, 148, 255]],
      "name": "YlGnBu",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/YlGnBu.png"
}, {
      "imageData": "",
      "colors": [[255, 255, 212, 255], [254, 217, 142, 255], [254, 153, 41, 255], [217, 95, 14, 255], [153, 52, 4, 255]],
      "name": "YlOrBr",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/YlOrBr.png"
}, {
      "imageData": "",
      "colors": [[255, 255, 178, 255], [254, 204, 92, 255], [253, 141, 60, 255], [240, 59, 32, 255], [189, 0, 38, 255]],
      "name": "YlOrRd",
      "url": "http://static.arcgis.com/images/ColorSets/Sequential/Multi/YlOrRd.png"
}, {
      "imageData": "",
      "colors": [[166, 97, 26, 255], [223, 194, 125, 255], [245, 245, 245, 255], [128, 205, 193, 255], [1, 133, 113, 255]],
      "name": "BrBG",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/BrBG.png"
}, {
      "imageData": "",
      "colors": [[208, 28, 139, 255], [241, 182, 218, 255], [247, 247, 247, 255], [184, 225, 134, 255], [77, 172, 38, 255]],
      "name": "PiYG",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/PiYG.png"
}, {
      "imageData": "",
      "colors": [[123, 50, 148, 255], [194, 165, 207, 255], [247, 247, 247, 255], [166, 219, 160, 255], [0, 136, 55, 255]],
      "name": "PRGn",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/PRGn.png"
}, {
      "imageData": "",
      "colors": [[230, 97, 1, 255], [253, 184, 99, 255], [247, 247, 247, 255], [178, 171, 210, 255], [94, 60, 153, 255]],
      "name": "PuOR",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/PuOR.png"
}, {
      "imageData": "",
      "colors": [[202, 0, 32, 255], [244, 165, 130, 255], [247, 247, 247, 255], [146, 197, 222, 255], [5, 113, 176, 255]],
      "name": "RdBu",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/RdBu.png"
}, {
      "imageData": "",
      "colors": [[202, 0, 32, 255], [244, 165, 130, 255], [255, 255, 255, 255], [186, 186, 186, 255], [64, 64, 64, 255]],
      "name": "RdGy",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/RdGy.png"
}, {
      "imageData": "",
      "colors": [[215, 25, 28, 255], [253, 174, 97, 255], [255, 255, 191, 255], [171, 217, 233, 255], [44, 123, 182, 255]],
      "name": "RdYlBu",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/RdYlBu.png"
}, {
      "imageData": "",
      "colors": [[215, 25, 28, 255], [253, 174, 97, 255], [255, 255, 191, 255], [166, 217, 106, 255], [26, 150, 65, 255]],
      "name": "RdYlGn",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/RdYlGn.png"
}, {
      "imageData": "",
      "colors": [[215, 25, 28, 255], [253, 174, 97, 255], [255, 255, 191, 255], [171, 221, 164, 255], [43, 131, 186, 255]],
      "name": "Spectral",
      "url": "http://static.arcgis.com/images/ColorSets/Diverging/Spectral.png"
}];
    
    function getColorRamp(colorRampName) {
      var colorRamp = array.filter(namedColorRamps, function(namedColorRamp) {
        return namedColorRamp.name === colorRampName;
      })[0];
      return colorRamp;
    }

    function buildColorRamp(colors, count, reverseRamp) {
      var i, ratio, c;
      // fill in colors in between  
      if (reverseRamp) {
        colors = lang.clone(colors);
        colors.reverse();
      }
      var buildList = function(colorA, colorB, count) {
        var outColors = [];
        if (count == 2) {
          outColors = [colorA, colorB];
        } else {
          for (i = 0; i < count; i++) {
            ratio = (1 / (count - 1)) * i;
            c = new Color([Math.round(colorA.r + ratio * (colorB.r - colorA.r)), Math.round(colorA.g + ratio * (colorB.g - colorA.g)), Math.round(colorA.b + ratio * (colorB.b - colorA.b))]);
            outColors.push(c);
          }
        }
        return outColors;
      };
      var outColors;
      if (count == 1) {
        outColors = [colors[0]];
      } else if (colors.length == 2) {
        if (count == 2) {
          outColors = [colors[0], colors[1]];
        } else {
          var colorA = new Color(colors[0]);
          var colorB = new Color(colors[1]);
          outColors = buildList(colorA, colorB, count);
        }
      } else {
        var color1 = colors[0];
        var color2 = colors[1];
        var color3 = colors[2];
        if (colors.length == 5) {
          color2 = colors[2];
          color3 = colors[4];
        }
        if (count == 2) {
          outColors = [color1, color3];
        } else if (count == 3) {
          outColors = [new Color(color1), new Color(color2), new Color(color3)];
        } else {
          var count_ = count;
          if (count % 2 === 0) {
            count_ = (count * 2) - 1;
          }
          outColors = buildList(new Color(color1), new Color(color2), Math.ceil(count_ / 2));
          var outColors2 = buildList(new Color(color2), new Color(color3), Math.ceil(count_ / 2));
          outColors2.splice(0, 1);
          outColors = outColors.concat(outColors2);
          if (count_ != count) {
            var list = [];
            for (i = 0; i < outColors.length; i++) {
              if (i % 2 === 0) {
                list.push(outColors[i]);
              }
            }
            outColors = list;
          }
        }
      }
      return outColors;
    }
    //////////////////////////////////////////
    //  Standard Deviation
    //////////////////////////////////////////
    /**
     * calculate the interval (0.25, 0.5, 1, ... stddev) for Standard Deviation
     */

    function calculateStdDevInterval(minValue, maxValue, breaks, mean, stddev) {
      var div = Math.max(mean - minValue, maxValue - mean);
      var stddevSteps = div / stddev;
      var interval = stddevSteps / breaks;
      if (interval >= 1.0) {
        interval = 1.0;
      } else if (interval >= 0.5) {
        interval = 0.5;
      } else {
        interval = 0.25;
      }
      return interval;
    }
    /**
     * calculate mean value
     */

    function getMean(values) {
      var i, mean = 0;
      for (i = 0; i < values.length; i++) {
        mean += values[i];
      }
      mean /= values.length;
      return mean;
    }
    /**
     * calculate standard deviation
     */

    function getStddev(values, mean) {
      var i, variant = 0, stddev, value;
      for (i = 0; i < values.length; i++) {
        value = values[i];
        variant += (value - mean) * (value - mean);
      }
      variant /= values.length;
      stddev = Math.sqrt(variant);
      return stddev;
    }
    //////////////////////////////////////////
    //  Natural Breaks (Jenk's) for ClassBreakRenderer
    //////////////////////////////////////////
    /**
     * calculate a list with positions that can be used to retrieve the values for the renderer breaks
     * uniqueValues: list of unique values
     * valueFrequency: list with frequency of unique values
     * breaks: number of breaks for renderer
     */

    function calcClassVariance(iClass, bList, uniqueValues, valueFrequency) {
      var sum = 0, count = 0, j, numdups;
      for (j = bList[iClass] + 1; j <= bList[iClass + 1]; j++) {
        numdups = valueFrequency[j];
        sum += uniqueValues[j] * numdups;
        count += numdups;
      }
      if (count <= 0) {
        console.log("Exception in Natural Breaks calculation");
      }
      var mean = sum / count;
      var sdcm = 0;
      for (j = bList[iClass] + 1; j <= bList[iClass + 1]; j++) {
        sdcm += valueFrequency[j] * Math.pow(uniqueValues[j] - mean, 2);
      }
      return {
        sbMean: mean,
        sbSdcm: sdcm
      };
    }
    
    function step1Optimization(bList, uniqueValues, valueFrequency, breaks) {
      var oldBreaks = []; // int values
      var oldmean = []; // var values
      var oldsdcm = []; // var values
      var sdcmVal = 0;
      var mean = [];
      var sdcm = [], i, j, obj;
      for (i = 0; i < breaks; i++) {
        obj = calcClassVariance(i, bList, uniqueValues, valueFrequency);
        mean.push(obj.sbMean);
        sdcm.push(obj.sbSdcm);
        sdcmVal += sdcm[i];
      }
      var oldsdcmVal = sdcmVal;
      var first = true;
      var value;
      while (first || sdcmVal < oldsdcmVal) { // MINIMIZE SDCM 
        first = false;
        oldBreaks = [];
        for (i = 0; i < breaks; i++) {
          oldBreaks.push(bList[i]);
        }
        for (i = 0; i < breaks; i++) {
          for (j = bList[i] + 1; j <= bList[i + 1]; j++) {
            value = uniqueValues[j];
            if ((i > 0) &&
              (j != bList[i + 1] &&
                (Math.abs(value - mean[i]) > Math.abs(value - mean[i - 1])))) {
              bList[i] = j;
              continue;
            }
            if ((i < breaks - 1) && (bList[i] != j - 1) && (Math.abs(value - mean[i]) > Math.abs(value - mean[i + 1]))) {
              bList[i + 1] = (j - 1);
              break;
            }
          }
        }
        oldsdcmVal = sdcmVal;
        sdcmVal = 0;
        oldmean = [];
        oldsdcm = [];
        for (i = 0; i < breaks; i++) {
          oldmean.push(mean[i]);
          oldsdcm.push(sdcm[i]);
          obj = calcClassVariance(i, bList, uniqueValues, valueFrequency);
          mean[i] = obj.sbMean;
          sdcm[i] = obj.sbSdcm;
          sdcmVal += sdcm[i];
        }
      }
      if (sdcmVal > oldsdcmVal) {
        for (i = 0; i < breaks; i++) {
          bList[i] = oldBreaks[i];
          mean[i] = oldmean[i];
          sdcm[i] = oldsdcm[i];
        }
        sdcmVal = oldsdcmVal;
      }
      return {
        mean: mean,
        sdcm: sdcm
      };
    }
    
    function step2Optimization(mean, sdcm, bList, uniqueValues, valueFrequency, breaks) {
      var newsdcm1 = 0;
      var newsdcm2 = 0;
      var newmean1 = 0;
      var newmean2 = 0;
      var somethingchanged = true, i, k, obj;
      for (k = 0; k < 2 && somethingchanged; k++) {
        if (k === 0) {
          somethingchanged = false;
        }
        // force down 
        for (i = 0; i < breaks - 1; i++) {
          while (bList[i + 1] + 1 != bList[i + 2]) {
            // while sdcm is decreasing and class we are taking from has more than one element 
            bList[i + 1] = bList[i + 1] + 1;
            obj = calcClassVariance(i, bList, uniqueValues, valueFrequency);
            newmean1 = obj.sbMean;
            newsdcm1 = obj.sbSdcm;
            obj = calcClassVariance(i + 1, bList, uniqueValues, valueFrequency);
            newmean2 = obj.sbMean;
            newsdcm2 = obj.sbSdcm;
            if (newsdcm1 + newsdcm2 < sdcm[i] + sdcm[i + 1]) {
              sdcm[i] = newsdcm1;
              sdcm[i + 1] = newsdcm2;
              mean[i] = newmean1;
              mean[i + 1] = newmean2;
              somethingchanged = true;
            } else {
              bList[i + 1] = bList[i + 1] - 1;
              break;
            }
          }
        }
        // force up 
        for (i = breaks - 1; i > 0; i--) {
          while (bList[i] != (bList[i - 1] + 1)) {
            // while sdcm is decreasing and class we are taking from has more than one element */
            bList[i] = bList[i] - 1;
            obj = calcClassVariance(i - 1, bList, uniqueValues, valueFrequency);
            newmean1 = obj.sbMean;
            newsdcm1 = obj.sbSdcm;
            obj = calcClassVariance(i, bList, uniqueValues, valueFrequency);
            newmean2 = obj.sbMean;
            newsdcm2 = obj.sbSdcm;
            if ((newsdcm1 + newsdcm2) < (sdcm[i - 1] + sdcm[i])) {
              sdcm[i - 1] = newsdcm1;
              sdcm[i] = newsdcm2;
              mean[i - 1] = newmean1;
              mean[i] = newmean2;
              somethingchanged = true;
            } else {
              bList[i] = bList[i] + 1;
              break;
            }
          }
        }
      }
      return somethingchanged;
    }
    
    function calculateBreakPositions(uniqueValues, valueFrequency, numBreaks) {
      var uniqueValuesSize = uniqueValues.length, i, bList = [];
      // No more Classes than unique Values
      if (numBreaks > uniqueValuesSize) {
        numBreaks = uniqueValuesSize;
      }
      for (i = 0; i < numBreaks; i++) {
        bList.push(Math.round((i * uniqueValuesSize / numBreaks) - 1));
      }
      bList.push((uniqueValuesSize - 1));
      var obj = step1Optimization(bList, uniqueValues, valueFrequency, numBreaks);
      if (step2Optimization(obj.mean, obj.sdcm, bList, uniqueValues, valueFrequency, numBreaks)) {
        obj = step1Optimization(bList, uniqueValues, valueFrequency, numBreaks);
      }
      return bList;
    }

    function calculateUniqueValues(values) {
      var uniqueValues = [];
      var valueFrequency = [];
      var lastVal = Number.MIN_VALUE;
      var curFreq = 1;
      var count = -1, i;
      for (i = 0; i < values.length; i++) {
        if (values[i] == lastVal) {
          curFreq++;
          valueFrequency[count] = curFreq;
        } else if (values[i] !== null) {
          uniqueValues.push(values[i]);
          lastVal = values[i];
          curFreq = 1;
          valueFrequency.push(curFreq);
          count++;
        }
      }
      return {
        uniqueValues: uniqueValues,
        valueFrequency: valueFrequency
      };
    }

    function calculateClassBreaks(features, values, fieldName, normFieldName, normType, method, numBreaks, total) {
      var i, feature, value, classBreaks = [], val, obj, bList, jumps, done, pos, mean, stddev;
  
      // Extract values from features if they were not passed in by the 
      // caller. See FeatureLayerStatistics._cbFromMemory also.
      if (!values) {
        values = [];
        
        for (i = 0; i < features.length; i++) {
          feature = features[i];
          value = feature.attributes[fieldName];
          if (normFieldName && (!normType || normType === "field")) {
            value /= feature.attributes[normFieldName];
          }
          else if (normType === "log") {
            value = Math.log(value)/Math.log(10);
          }
          else if (normType === "percent-of-total" && total) {
            value *= 100;
            value /= total;
          }
          
          if (esriLang.isDefined(value)) {
            if (isNaN(value)) {
              return null;
            }
            values.push(parseFloat(value));
          }
        }
      }
      
      if (values.length === 0) {
        return [];
      }
      
      // only number fields
      var sortfunction = function(a, b) {
        return (a - b);
      };
      values = values.sort(sortfunction);
      
      var min, max, interval, lastVal;
      if (method.toLowerCase() == "equal-interval") {
        if (values.length >= numBreaks) {
          min = values[0];
          max = values[values.length - 1];
          interval = (max - min) / numBreaks;
          lastVal = min;
          for (i = 1; i < numBreaks; i++) {
            val = number.round((min + (i * interval)), 6);
            classBreaks.push({
              minValue: lastVal,
              maxValue: val
            });
            lastVal = val;
          }
          classBreaks.push({
            minValue: lastVal,
            maxValue: max
          });
        } else {
          // we don't have enough values
          array.forEach(values, function(value) {
            classBreaks.push({
              minValue: value,
              maxValue: value
            });
          }, this);
        }
      } else if (method.toLowerCase() == "natural-breaks") {
        if (values.length > 0) {
          min = values[0];
          max = values[values.length - 1];
          obj = calculateUniqueValues(values);
          bList = calculateBreakPositions(obj.uniqueValues, obj.valueFrequency, numBreaks);
          lastVal = min;
          for (i = 1; i < numBreaks; i++) {
            if (obj.uniqueValues.length > i) {
              val = number.round(obj.uniqueValues[bList[i]], 6);
              classBreaks.push({
                minValue: lastVal,
                maxValue: val
              });
              lastVal = val;
            }
          }
          classBreaks.push({
            minValue: lastVal,
            maxValue: max
          });
        }
      } else if (method.toLowerCase() == "quantile") {
        if (values.length > 0) {
          min = values[0];
          max = values[values.length - 1];
          if (values.length >= numBreaks && min !== max) {
            lastVal = min;
            jumps = Math.ceil(values.length / numBreaks);
            done = 0;
            for (i = 1; i < numBreaks; i++) {
              pos = (jumps + done) - 1;
              if (pos > values.length) {
                pos = values.length - 1;
              }
              if (pos < 0) {
                pos = 0;
              }
              classBreaks.push({
                minValue: lastVal,
                maxValue: values[pos]
              });
              lastVal = values[pos];
              done += jumps;
              jumps = Math.ceil((values.length - done) / (numBreaks - i));
            }
            classBreaks.push({
              minValue: lastVal,
              maxValue: max
            });
          } else {
            // we don't have enough values or all values are the same
            // values are sorted, but we might have doubles. delete them
            lastVal = -1;
            for (i = 0; i < values.length; i++) {
              // don't round, otherwise we won't find the same values anymore
              val = values[i];
              if (val != lastVal) {
                lastVal = val;
                classBreaks.push({
                  minValue: lastVal,
                  maxValue: val
                });
                lastVal = val;
              }
            }
          }
        }
      } else if (method.toLowerCase() == "standard-deviation") {
        mean = getMean(values);
        stddev = getStddev(values, mean);
        if (stddev === 0) { // all values are the same
          classBreaks.push({
            minValue: values[0],
            maxValue: values[0]
          });
        } else {
          min = values[0];
          max = values[values.length - 1];
          interval = calculateStdDevInterval(min, max, numBreaks, mean, stddev);
          var numClasses = 0;
          var classSize = interval * stddev;
          lastVal = min;
          for (i = numBreaks; i >= 1; i--) {
            val = number.round((mean - ((i - 0.5) * classSize)), 6);
            classBreaks.push({
              minValue: lastVal,
              maxValue: val
            });
            lastVal = val;
            numClasses++;
          }
          // Mean
          val = number.round((mean + 0.5 * classSize), 6);
          classBreaks.push({
            minValue: lastVal,
            maxValue: val
          });
          lastVal = val;
          numClasses++;
          for (i = 1; i <= numBreaks; i++) {            
            if (numClasses == numBreaks * 2) {
              val = max;
            } else {
              val = number.round((mean + ((i + 0.5) * classSize)), 6);
            }
            classBreaks.push({
              minValue: lastVal,
              maxValue: val
            });
            lastVal = val;
            numClasses++;
          }
        }
      }
      return classBreaks;
    }

    function createClassBreaksRenderer(params) {
      var features = params.features,
          fieldName = params.definition.classificationField,
          normFieldName = params.definition.normalizationField,
          normType = params.definition.normalizationType,
          method = params.definition.classificationMethod,
          count = params.definition.breakCount,
          defaultSymbol,
          colorRampName = params.definition.colorRamp || "BuGn";
      if (params.definition.baseSymbol) {
        defaultSymbol = params.definition.baseSymbol;
      }
      else {
        switch (features[0].geometry.type) {
          case "point":
          case "multipoint":
            defaultSymbol = new SimpleMarkerSymbol();
            break;
          case "polyline":
            defaultSymbol = new SimpleLineSymbol();
            break;
          case "polygon":
            defaultSymbol = new SimpleFillSymbol();
            break;
        }        
      }
      var total = 0;
      if (normType === "percent-of-total") {
        array.forEach(features, function(feature){
          total += feature.attributes[fieldName];
        });
      }
      var breaks = calculateClassBreaks(features, params.values, fieldName, normFieldName, normType, method, count, total);
      var renderer = new ClassBreaksRenderer(defaultSymbol, fieldName);
      if (normType) {
        renderer.normalizationType = normTypes[normType];
      }
      renderer.classificationMethod = methods[method];
      //create color from namedColorRamp
      var colorRamp = getColorRamp(colorRampName);
      var colors;
      if (colorRamp) {
        colors = buildColorRamp(colorRamp.colors, count);
      } else {
        return;
      }
      
      var label;
      array.forEach(breaks, function(item, idx) {
        if (item.minValue === item.maxValue) {
          if (normType && normType === "percent-of-total") {
            label = item.minValue + "%";
          }
          else {
            label = item.minValue.toString();
          }
        }
        else {
          if (normType && normType === "percent-of-total") {
            label = item.minValue + "% - " + item.maxValue + "%";
          }
          else {
            label = item.minValue + " - " + item.maxValue;
          }
        }
        renderer.addBreak({
          minValue: item.minValue,
          maxValue: item.maxValue,
          symbol: lang.clone(defaultSymbol).set("color", colors[idx]),
          label: label
        });
      });
      
      if (normType === "percent-of-total") {
        renderer.normalizationTotal = total;
        //it has to recreate the renderer, otherwise it won't be applied.
        //need to investigate more why this happens.
        renderer = new ClassBreaksRenderer(renderer.toJSON());
      }
      
      renderer.setMaxInclusive(true);

      return renderer;
    }
    
    function createUniqueValueRenderer(params){
      var features = params.features,
          fieldName = params.definition.attributeField,
          defaultSymbol,
          colorRampName = params.definition.colorRamp || "BuGn";
      if (params.definition.baseSymbol) {
        defaultSymbol = params.definition.baseSymbol;
      }
      else {
        switch (features[0].geometry.type) {
          case "point":
          case "multipoint":
            defaultSymbol = new SimpleMarkerSymbol();
            break;
          case "polyline":
            defaultSymbol = new SimpleLineSymbol();
            break;
          case "polygon":
            defaultSymbol = new SimpleFillSymbol();
            break;
        }        
      }
      //get and sort values
      var values = [], i, feature, value;
      for (i = 0; i < features.length; i++) {
        feature = features[i];
        value = feature.attributes[fieldName];
        values.push(value);
      }
      if (values.length === 0) {
        return;
      }

      values = values.sort();
      var uniqueValues = calculateUniqueValues(values).uniqueValues;
      var count = uniqueValues.length;
      var renderer = new UniqueValueRenderer(defaultSymbol, fieldName);
      //create color from namedColorRamp
      var colorRamp = getColorRamp(colorRampName);
      var colors;
      if (colorRamp) {
        colors = buildColorRamp(colorRamp.colors, count);
      } else {
        return;
      }
      
      array.forEach(uniqueValues, function(item, idx) {
        renderer.addValue({
          value: item,
          symbol: lang.clone(defaultSymbol).set("color", colors[idx]),
          label: item
        });
      });

      return renderer;
    }
    
    var generateRenderer = {
      createClassBreaksRenderer: createClassBreaksRenderer,
      createUniqueValueRenderer: createUniqueValueRenderer,
      getColorRamp: getColorRamp
    };
    
    return generateRenderer;
  });
