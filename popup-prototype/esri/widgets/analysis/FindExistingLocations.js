/******************************************
  esri/widgets/analysis/FindExistingLocations
******************************************/
define(
[ 
  "require",
  ...
  "./ExpressionGrid",
  "dojo/i18n!../../nls/jsapi",
  "dojo/text!./templates/FindExistingLocations.html"
], function(require, declare, lang, array, connection, has, string, domStyle, domAttr, domConstruct, query, domClass, _WidgetBase,
   _TemplatedMixin, _WidgetsInTemplateMixin,  _OnDijitClickMixin, _FocusMixin, registry, Button, CheckBox, Form, Select, TextBox, ValidationTextBox, ContentPane, ComboBox, Dialog, esriKernel, esriLang, AnalysisBase, _AnalysisOptions, AnalysisUtils, CreditEstimator, ExpressionGrid, jsapiBundle, template) {
  var FindExistingLocations = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin,  _OnDijitClickMixin, _FocusMixin, _AnalysisOptions, AnalysisBase], {

    declaredClass: "esri.widgets.analysis.FindExistingLocations",
    ...
    postMixInProperties: function(){
      this.inherited(arguments);
      lang.mixin(this.i18n, jsapiBundle.findExistingLocations);
    ...
    _handleSaveBtnClick: function() {
      ...
      descriptionStr = string.substitute(this.i18n.itemDescription, {analysisLayerName: this.analysisLayer.name});
    ...
    _handleShowCreditsClick: function(e) {
      e.preventDefault();
      var jobParams = {}, contextObj, expMap, inputLayers;
      if(!this._form.validate() || !this.expressionGrid.validate()) {
    ...
    _handleUpdateExpressions: function(items) {
      //console.log(items);
      if(items.length > 1) {
        domStyle.set(this._showCreditsLink, "color", "");
        domStyle.set(this._showCreditsLink, "cursor", "");
    ...
    _updateAnalysisLayerUI: function(override) {
      if(this.analysisLayer) {
        ...
        this._outputLayerInput.set("value", this.outputLayerName);       
        domAttr.set(this._findExpLabel, "innerHTML", this.i18n.findExpLabel);
      ...
      if(value && value.selection) {
        AnalysisUtils.addAnalysisReadyLayer({
          ...
          browseDialog: this._browsedlg
            ...
            if(this._isAnalysisSelect) {
              this.analysisLayer = this.analysisLayers[this._analysisSelect.get("value")];
              this.analysisLayer.id = this.analysisLayer.id + "";
              if(!this.inputLayers[this._analysisSelect.get("value")]) {
                this.inputLayers.push(this.analysisLayer);
              }
              this._updateAnalysisLayerUI(true);
            }
        }));
