define([
    ...
    "dojo/text!./ClassedSizeSlider/templates/ClassedSizeSlider.html"
  ],
  function (
    ...
    var ClassedSizeSlider = declare("esri.widgets.ClassedSizeSlider", [_WidgetBase, _TemplatedMixin, _OnDijitClickMixin, Evented], {
      
      // Dom Node Properties
      baseClass: "esriClassedSizeSlider",
      basePath: require.toUrl("esri/widgets/ClassedSizeSlider/"),
      ...
      startup: function (){
        ...
        // Sync _slider with ClassedSizeSlider instance
      ...
      // Params:
      // -- breakInfos constructor param
      // Returns:
      // -- updated values with handle logic included
      ...
      _updateBreakInfoLabels: function(){
        ...
        /*
        // Update label based on current min/max values
      ...
      // -- Fills the ramp
      // Notes:
      // -- Probably not needed for Classed Size
      // -- Rewrite function to handle pie generation per break
      _fillRamp: function(){
        ...
        array.forEach(breakInfos, function(breakInfo){
          stepPoints.push([
            height - Math.round((breakInfo.minValue - slider.minimum) / (slider.maximum - slider.minimum) * height),
            height - Math.round((breakInfo.maxValue - slider.minimum) / (slider.maximum - slider.minimum) * height)
          ]);
        ...
        var stepWidth = this.rampWidth / breakInfos.length;
        var stepWidthTracker = this.rampWidth;
        var stepsPath = this._colorRampSurface.createPath().moveTo(stepWidthTracker, 0);

        array.forEach(stepPoints, lang.hitch(this,function(step, i){
          stepsPath.lineTo(stepWidthTracker, step[0]);
          stepWidthTracker = this.rampWidth - (stepWidth * (i +1));
          stepsPath.lineTo(stepWidthTracker, step[0]);
        }));

        stepsPath.lineTo(1, height)
          .lineTo(0, height)
          .lineTo(0, 0)
          .closePath()
          //.setFill("#007ac2");
