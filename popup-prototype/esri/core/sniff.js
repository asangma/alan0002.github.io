...
function(has, esriKernel) {
  ...
  esriKernel._getDOMAccessor = function(propName) {
    var prefix = "";
  ...
  esriKernel._css = function(force3D) {
    ...
    return {
      ...
      matrix: function(m) {
        if (m.m) {
          m = m.m;
          // esri.views.2d.Matrix2D
          return matrixPrefix + m[0].toFixed(10) + "," + m[1].toFixed(10) + matrixC1 +  
               m[2].toFixed(10) + "," + m[3].toFixed(10) + matrixC2 + 
               m[4].toFixed(10) + (isFF ? "px," : ",") + m[5].toFixed(10) + (isFF ? "px" : "") +
               matrixSuffix;
        } else {
          ...
          return matrixPrefix + m.xx.toFixed(10) + "," + m.yx.toFixed(10) + matrixC1 +  
                 m.xy.toFixed(10) + "," + m.yy.toFixed(10) + matrixC2 + 
      ...
      matrix3d: function(m) {
        m = m.m;
        return "matrix3d("+m[0].toFixed(10)+","+m[1].toFixed(10)+",0,0,"
                          +m[2].toFixed(10)+","+m[3].toFixed(10)+",0,0,0,0,1,0,"
                          +m[4].toFixed(10)+","+m[5].toFixed(10)+",0,1)";
      },
  ...
  var detectWebGL = function () {
    ...
    try {
      ...
      // Browser does not support WebGL at all
      return false;
    }

    var gl = create3DContext(canvas);
    if (!gl) {
      // Browser seems to have some support but doesn't allow to create a WebGL canvas
      return false;
    }
  ...
  has.add("esri-webgl", detectWebGL);
