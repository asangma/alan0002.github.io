...
[
  ...
  declare, lang, connect, array, Menu, MenuItem,
  VertexMover, Point, jsonUtils, jsapiBundle
) {
  var GVE = declare(null, {
    ...
    _addControllers: function() {
      this._firstMoveHandle = connect.connect(VertexMover, "onFirstMove", this, this._firstMoveHandler);
      this._moveStopHandle = connect.connect(VertexMover, "onMoveStop", this, this._moveStopHandler);
      ...
      this._mouseOverHandle = connect.connect(graphicsLayer, "onMouseOver", this, this._mouseOverHandler);
      this._mouseOutHandle = connect.connect(graphicsLayer, "onMouseOut", this, this._mouseOutHandler);

      if (this._canDel) {
        // create right-click context menu for existing vertices
        this._ctxMenu = new Menu({ style: "font-size: 12px; margin-left: 5px; margin-top: 5px;" });
        var menuItem = (this._ctxDelete = new MenuItem({ label: jsapiBundle.toolbars.edit.deleteLabel, iconClass: "vertexDeleteIcon", style: "outline: none;" }));
        this._deleteHandle = connect.connect(menuItem, "onClick", this, this._deleteHandler);
    ...
    _add: function(segments, symbol, placeholders) {
      ...
      for (i = 0; i < segments.length; i++) {
        ...
        for (j = 0; j < segment.length; j++) {
          group.push(
            new VertexMover(segment[j], symbol, graphic, i, j, segment.length, this, placeholders)
          );
        }
        movers.push(group);
      }
    ...
    _remove: function(movers) {
      if (movers) {
        array.forEach(movers, function(group) {
          array.forEach(group, function(mover) {
            mover.destroy();
          });
        });
      }
    ...
    _isNew: function(mover) {
      return (array.indexOf(this._vertexMovers[mover.segIndex], mover) === -1) ? true : false;
    },
  ...
  //    }
    ...
    _findMover: function(graphic) {
      ...
      array.forEach(this._vertexMovers, function(group) {
        movers = movers.concat(group);
      });

      if (mpMovers) {
        array.forEach(mpMovers, function(group) {
          movers = movers.concat(group);
        });
      }
    ...
    _repositionRelatedMidpoints: function(mover) {
      ...
      for (i = 0; i < indices.length; i++) {
        ...
        var midpoint = new Point({ x: (point1.x + point2.x ) / 2, y: (point1.y + point2.y ) / 2, spatialReference: point1.spatialReference.toJSON() });
    ...
    _addMidpoints: function(mover) {
      ...
      var midpoint1 = new Point({ x: (point1.x + point2.x ) / 2, y: (point1.y + point2.y ) / 2, spatialReference: point1.spatialReference.toJSON() });
      ...
      var midpoint2 = new Point({ x: (point1.x + point2.x ) / 2, y: (point1.y + point2.y ) / 2, spatialReference: point1.spatialReference.toJSON() });
      
      var mvr1 = new VertexMover(midpoint1, this._symbol2, this.graphic, mover.segIndex, ptIndex, newLength, this, true);
      var mvr2 = new VertexMover(midpoint2, this._symbol2, this.graphic, mover.segIndex, ptIndex + 1, newLength, this, true);
  ...
  lang.mixin(GVE, {
    create: function(graphic, map, toolbar) {
      ...
      switch(type) {
        case "multipoint":
          return new GVE.MultipointVertexEditor(graphic, map, toolbar);
        case "polyline":
          return new GVE.PolylineVertexEditor(graphic, map, toolbar);
        case "polygon":
          return new GVE.PolygonVertexEditor(graphic, map, toolbar);
      }
    }
  });

  GVE.MultipointVertexEditor = declare(GVE, {
    ...
    constructor: function() {
      this._moveStartHandle = connect.connect(VertexMover, "onMoveStart", this, this._moveStartHandler);
      connect.disconnect(this._firstMoveHandle);
    },
    ...
    _getSegments: function(geometry) {
      ...
      for (i = 0; i < points.length; i++) {
        var point = points[i];
        segment.push(new Point({ x: point[0], y: point[1], spatialReference: sr.toJSON() }));
      }
      return [ segment ];
    },
  ...
  GVE.PolylineVertexEditor = declare(GVE, {
    ...
    _getMidpointSegments: function(geometry) {
      ...
      for (i = 0; i < paths.length; i++) {
        ...
        for (j = 0; j < path.length - 1; j++) {
          ...
          var midpoint = new Point({ x: midX, y: midY, spatialReference: sr.toJSON() });
    ...
    _deleteMidpoints: function(mover) {
      ...
      if (indices.length === 1) { // deleting first or last vertex
        ...
        var midpoint1 = new Point({ x: (point1.x + point2.x ) / 2, y: (point1.y + point2.y ) / 2, spatialReference: point1.spatialReference.toJSON() });
        var mvr1 = new VertexMover(midpoint1, this._symbol2, this.graphic, mover.segIndex, min, newLength, this, true);
    ...
    _updateRelatedGraphic: function(mover, graphic, newPoint, segIndex, ptIndex, segLen, add, del) {
      ...
      if (add) {
        geom.insertPoint(segIndex, ptIndex + 1, jsonUtils.fromJSON(newPoint.toJSON()));
      }
      else if (del) {
        geom.removePoint(segIndex, ptIndex);
      }
      else {
        geom.setPoint(segIndex, ptIndex, jsonUtils.fromJSON(newPoint.toJSON()));
      }
  ...
  GVE.PolygonVertexEditor = declare(GVE, {
    ...
    _deleteMidpoints: function(mover) {
      ...
      if (ptIndex === 0) {
        ...
        midpoint1 = new Point({ x: (point1.x + point2.x ) / 2, y: (point1.y + point2.y ) / 2, spatialReference: point1.spatialReference.toJSON() });
        mvr1 = new VertexMover(midpoint1, this._symbol2, this.graphic, mover.segIndex, newLength - 1, newLength, this, true);
        ...
        point2 = mover.relatedGraphic.geometry.getPoint(mover.segIndex, verts[1]);
        midpoint1 = new Point({ x: (point1.x + point2.x ) / 2, y: (point1.y + point2.y ) / 2, spatialReference: point1.spatialReference.toJSON() });
        mvr1 = new VertexMover(midpoint1, this._symbol2, this.graphic, mover.segIndex, min, newLength, this, true);
        var spliced = movers.splice(min, indices.length, mvr1);
        for (i = 0; i < spliced.length; i++) {
          spliced[i].destroy();
        }
    ...
    _updateRelatedGraphic: function(mover, graphic, newPoint, segIndex, ptIndex, segLen, add, del) {
      ...
      if (add) {
        ...
        if (ptIndex === 0) {
          ...
          geom.setPoint(segIndex, segLen - 1, jsonUtils.fromJSON(geom.getPoint(segIndex, 0).toJSON()));
        }
      }
      else {
        geom.setPoint(segIndex, ptIndex, jsonUtils.fromJSON(newPoint.toJSON()));
        if (ptIndex === 0) { // polygons are "closed", remember? first & last points must be updated together
          geom.setPoint(segIndex, segLen, jsonUtils.fromJSON(newPoint.toJSON()));
        }
      }
