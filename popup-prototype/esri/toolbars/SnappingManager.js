...
function(
  declare, connection, lang, array, Color, Deferred, has, djKeys,
  Graphic, ScreenPoint, Extent, 
  SimpleMarkerSymbol, SimpleLineSymbol, Query
) {

//dojo.require("esri.tasks.query");

var SnappingManager = declare(null, {
  ...
  constructor: function (params) {
    ...
    if (params.layerInfos) {
      ...
      if (this.map.loaded) {
        ...
        var loadHandler = connection.connect(this.map, "onLoad", this, function(map){
          connection.disconnect(loadHandler);
          loadHandler = null;
          this.layerInfos.push({
            "layer": this.map.graphics
          });
    ...
    if (params.snapPointSymbol) {
      ...
      this.snapPointSymbol = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CROSS, 15, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([0, 255, 255]), 1), new Color([0, 255, 0, 0]));
    }
    ...
    if (params.snapKey) {
      ...
      this.snapKey = (has("mac")) ? djKeys.META : djKeys.CTRL;
    }
    this._SelectionLyrQuery = new Query();
    this._SelectionLyrQuery.spatialRelationship = Query.SPATIAL_REL_INTERSECTS;
    this._snappingGraphic = new Graphic();
    this.setLayerInfos(this.layerInfos);
  ...
  getSnappingPoint: function (screenPoint) {
    var layers = this.layers,
        ...
        queryExtent = new Extent(bottomLeft.x, bottomLeft.y, topRight.x, topRight.y, map.spatialReference),
        query = new Query();
    ...
    array.forEach(layers, function(layer, idx){
      if (layer.visible && layer.loaded && layer.declaredClass === "esri.layers.FeatureLayer" && layer.mode !== layer.constructor.MODE_SELECTION) {
        count++;
      }
    });
    
    if (map.spatialReference.isWrappable()) {
      xmin = Extent.prototype._normalizeX(queryExtent.xmin, map.spatialReference._getInfo()).x;
      xmax = Extent.prototype._normalizeX(queryExtent.xmax, map.spatialReference._getInfo()).x;
    }
    var graphicsQueryExtent = new Extent(xmin, queryExtent.ymin, xmax, queryExtent.ymax, map.spatialReference);
    ...
    array.forEach(layers, function (layer, idx) {
      if (layer.visible && layer.loaded) {
        layerIdx = idx;
        if ((layer.declaredClass === "esri.layers.FeatureLayer") && layer.mode !== layer.constructor.MODE_SELECTION) {
          happend = true;
  ...
  setLayerInfos: function (layerInfos) {
    ...
    array.forEach(this.layers, function (layer, idx) {
      if (layer.declaredClass === "esri.layers.FeatureLayer" && layer.mode === layer.constructor.MODE_SELECTION) {
  ...
  destroy: function () {
    connection.disconnect(this._onExtentChangeConnect);
  ...
  _startSelectionLayerQuery: function () {
    ...
    this._onExtentChangeConnect = connection.connect(this.map, "onExtentChange", lang.hitch(this, "_mapExtentChangeHandler", this._selectionLayers, this._selectionLayerOptions));
  },
  ...
  _extractPointsAndLines: function (features, layerOption) {
    var featurePts = [],
        featureLines = [];
    var i, j;
    array.forEach(features, function (feature, idx) {  
      //can get psuedo feature from index (assume visible)
      if ((!feature._graphicsLayer || feature.visible) && feature.geometry) {
  ...
  _getSnappingPoint: function (featurePts, featureLines, screenPoint) {
    ...
    var pointX, pointY;
    array.forEach(featurePts, function (pt, idx) {
    ...
    if (pointX) {
      var snappingVertexPoint = new ScreenPoint(pointX, pointY);
      ...
      if (edgePointX) {
        var edgePoint = new ScreenPoint(edgePointX, edgePointY);
  ...
  _setUpSnapping: function () {
    var map = this.map;
    this._onSnapKeyDown_connect = connection.connect(map, "onKeyDown", this, "_onSnapKeyDownHandler");
    this._onSnapKeyUp_connect = connection.connect(map, "onKeyUp", this, "_onSnapKeyUpHandler");
    this._onSnappingMouseMove_connect = connection.connect(map, "onMouseMove", this, "_onSnappingMouseMoveHandler");
    this._onSnappingMouseDrag_connect = connection.connect(map, "onMouseDrag", this, "_onSnappingMouseMoveHandler");
    if (this.alwaysSnap) {
  ...
  _killOffSnapping: function () {
    connection.disconnect(this._onSnapKeyDown_connect);
    connection.disconnect(this._onSnapKeyUp_connect);
    connection.disconnect(this._onSnappingMouseMove_connect);
    connection.disconnect(this._onSnappingMouseDrag_connect);
  ...
  _onSnapKeyUpHandler: function (evt) {
    if (evt.keyCode === this.snapKey) {
      this._onSnapKeyDown_connect = connection.connect(this.map, "onKeyDown", this, "_onSnapKeyDownHandler");
      if (this.alwaysSnap) {
        this._activateSnapping();
      } else {
        this._deactivateSnapping();
  ...
  _onSnappingMouseMoveHandler: function (evt) {
    ...
    if (this._snappingActive) {
      ...
      deferred.then(this._snappingCallback);
    }
  },
