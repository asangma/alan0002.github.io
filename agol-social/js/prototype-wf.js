...
function populateFeed(data) {
    ...
    jQuery(data).each(function(index, date) {
        ...
        html_str += '<h2 class="'+ dateClass +'"><span>' + date_str + '</span></h2>';
        ...
        jQuery(date.events).each(function(i, event) {
            ...
            html_str += '<div class="feed-event '+oddEven+' '+ first +'" id="'+ eventID +'" >'; //  [ feed event

                /*html_str += '<div class="event-icon-wrap '+ typeClass +'">'; // [  icon
                    html_str += '<i class="esri-icon-' + typeClass + ' event-icon"></i>';
                html_str += '</div>';   //  icon  ]*/
                html_str += '<div class="event-content-wrap">'; //  [ content
                    html_str += '<div class="event-meta clearfix">'; // [ meta
                        //html_str += '<div class="item-thumb showing"><img src="' + event.itemThumb + '"/></div>';
                        html_str += '<div class="item-thumb showing"><img src="http://placehold.it/200x133/959595/ffffff"/></div>';
                        ...
                        html_str += '</div>';
                            if(event.followID) {
                                var show_str = (event.following) ? 'Following' : 'Don\'t show me things like this.';
                                var unshow_str = (event.following) ? 'Unfollow' : 'Don\'t show me things like this.';
                    ...
                    html_str += '<div class="event-info">'; //  [ info
                        html_str += '<div class="event-actor">'; //  [ actor
                            html_str += '<img class="event-actor-avatar" src="http://placehold.it/30/959595/000000"/> <a href="#">' + event.actor + '</a>';
                        html_str += '</div>';   //  actor   ]
                            ...
                            html_str += '<ul>';
                                ...
                                jQuery(event.actions).each(function(index, el) {
                                    ...
                                    if(numActions > maxShowingActions && (index+1) == numActions) {
                                        html_str += '</div>';
                                        html_str += '<li style="list-style:none;"><a href="javascript:;" class="reveal-hidden-actions" data-hidden-actions="action_'+ i +'"><span class="show-actions"><i class="esri-icon-down-arrow"></i> Show '+ numHiddenActions +' more</span><span class="hide-actions"><i class="esri-icon-up-arrow"></i> Hide</span></a></li>';
                                    }
                                });
                            html_str += '</ul>';
                        html_str += '</div>'; //    desc ]
                        ...
                        html_str += '<hr/>';
                        html_str += '<div class="event-time"><i class="icon-time"></i> ' + event.actionTime + '</div>';    //  [ time ]
                    html_str += '</div>'; //    info ]
