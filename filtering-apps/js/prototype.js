...
function getData() {
    jQuery.get('data/templates.json', function(data, status, xhr) {
        templates_obj = data;
        templateHtml = '';
...
function popuplateFilters() {
    ...
    jQuery(templates_obj).each(function(i, category){
        ...
        var href = (i > 0) ? category.id : 'everything';
        var title = (i > 0) ? category.title : 'Everything';
        filterHtml += '<li class="'+ active +'"><a class="filter" href="'+ href +'">'+ title +'</a></li>';
    });
    jQuery('#filters').html(filterHtml);
}


function filterTemplates(filter, is_everything) {
    if(filter !== 'everything') {
        ...
        templateHtml = '<p></p>';
        jQuery(templates_obj).each(function(index, category){
            if(category.id !== 'everything') filterTemplates(category.id, true);
        });
    }
    ...
    // add loader
    clearTimeout(timer);
    if(!jQuery('#loader-wrap').length) jQuery('.template-holder').append('<div id="loader-wrap"><img src="images/LoadingIndicator_blue.gif"><br/>*(and loaders for individual template thumbs)</div>');
...
function setControls() {
    jQuery('.filter').on('click', function(event) {
        ...
        jQuery('#template-info-panel').removeClass('showing');
...
function addTemplateListener() {
    ...
    jQuery('.template').on('click', function(event) {
        ...
        jQuery('#template-info-panel').addClass('showing');
...
function addOptionsListeners() {
    ...
    jQuery('.toggle-pagination').on('click', function(event){
        event.preventDefault();
        jQuery('#templates').css('opacity', '0');
        jQuery('.template-holder').toggleClass('no-scroll');
        jQuery('#template-info-panel').removeClass('showing');
        ...
        numTemplates = 0;
        jQuery('.tabs-left .active').removeClass('active');
        jQuery('#filters:first-child').addClass('active');
        filterTemplates('everything');
    });
}